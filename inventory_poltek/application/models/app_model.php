...
class App_Model extends CI_Model {
	...
	public function mahasiswa($id){
		$t = "SELECT * FROM mahasiswa WHERE nim='$id'";
		$d = $this->app_model->manualQuery($t);
		$r = $d->num_rows();
		if($r>0){
			foreach($d->result() as $h){
	...
	public function CariJmlBeli($kode,$kode1,$tgl_awal,$tgl_akhir){
		$t = "SELECT a.kode_barang,sum(a.jmlbeli) as jml
				FROM d_beli as a
				JOIN h_beli as b
				ON a.kodebeli = b.kodebeli
				WHERE a.kode_barang='$kode' and b.kodebeli ='$kode1' 
	...
	public function CariJmlJual($kode,$kode1,$tgl_awal,$tgl_akhir){
		$t = "SELECT a.kode_barang,sum(a.jmljual) as jml 
				FROM d_jual as a 
				JOIN h_jual as b
				ON a.kodejual = b.kodejual
				WHERE a.kode_barang='$kode' and b.kodejual ='$kode1'
	...
	public function CariStokAkhir($kode){
		...
		$beli = $this->app_model->CariJmlBeli($kode,$kode1,$tgl,$tgl);
		$jual = $this->app_model->CariJmlJual($kode,$tgl,$tgl);
		$hasil = ($beli-$jual);
		return $hasil;
	}

	public function CariStokAkhir1($kode){
		date_default_timezone_set('Asia/Jakarta');
		$tgl = date('Y-m-d');
		$awal = $this->app_model->CariStokAwal($kode);
