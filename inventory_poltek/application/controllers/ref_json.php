	...
	public function InfoBarang()
	{
		...
		if(!empty($cek)){
			...
			if($this->session->userdata('level')=='User'){
				...
				$text = "SELECT kode_barang,nama_barang FROM barang WHERE kode_barang='$kode' AND id_gudang='$gudang'";
			}else{
				$text = "SELECT kode_barang,nama_barang FROM barang WHERE kode_barang='$kode'";
			}
	...
	public function DataBarang(){
		...
		if(!empty($cek)){
			...
			if($this->session->userdata('level')=='User'){
				$gudang = $this->session->userdata('gudang');
				if(empty($cari)){
					$text = "SELECT kode_barang,nama_barang FROM barang WHERE id_gudang='$gudang'";
				}else{
					$text = "SELECT kode_barang,nama_barang FROM barang 
					WHERE id_gudang='$gudang' AND (kode_barang LIKE '%$cari%' OR nama_barang LIKE '%$cari%')";
				}
			}else{
				if(empty($cari)){
					$text = "SELECT kode_barang,nama_barang FROM barang";
		...
		if(!empty($cek)){
			...
			if($this->session->userdata('level')=='User'){
				...
				if(empty($cari)){
					$text = "SELECT *
					FROM d_beli as y
					left join h_beli as z
					on y.kodebeli=z.kodebeli
